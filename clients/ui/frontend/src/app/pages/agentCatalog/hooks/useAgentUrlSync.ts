...
import { AGENT_FILTER_KEYS } from '~/app/pages/agentCatalog/const';
...
function filtersFromParams(params: URLSearchParams): AgentCatalogFiltersState {
  const filters: AgentCatalogFiltersState = {};
...
function filtersToParams(filters: AgentCatalogFiltersState, params: URLSearchParams): void {
  for (const key of AGENT_FILTER_KEYS) {
...
type UseAgentUrlSyncReturn = {
  initialState: UrlState;
  syncToUrl: (state: UrlState) => void;
};

export function useAgentUrlSync(): UseAgentUrlSyncReturn {
