...
function isAgentFilterStringOption(v: unknown): v is CatalogFilterStringOption {
...
export function mapBackendFilterOptions(
  ...
  const mapped: AgentCatalogFilterOptionsList['filters'] = {};
  for (const [key, value] of Object.entries(raw.filters)) {
    const frontendKey = BACKEND_TO_FRONTEND_AGENT_FILTER_KEY[key] ?? key;
    if (isAgentFilterCategoryKey(frontendKey) && isAgentFilterStringOption(value)) {
...
type State = AgentCatalogFilterOptionsList | null;
